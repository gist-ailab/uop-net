import os
import sys
sys.path.append("/SSDe/sangjun_noh/workspace/ral_uop_rebuttal")


from openpoints.models.layers import furthest_point_sample, random_sample, LocalAggregation, create_convblock2d, three_interpolate, \
...
class PointMLPForUOP(nn.Module):
    def __init__(self, in_channels=3, embed_dim=128, groups=1, res_expansion=1.0,
                 activation="relu", bias=False, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[24, 24, 24, 24], reducers=[2, 2, 2, 2], **kwargs):
    
        super(PointMLPForUOP, self).__init__()
        self.stages = len(pre_blocks)
        self.embedding = ConvBNReLU1D(in_channels, embed_dim, bias=bias, activation=activation)
        assert len(pre_blocks) == len(k_neighbors) == len(reducers) == len(pos_blocks) == len(dim_expansion), \
            "Please check stage number consistent for pre_blocks, pos_blocks k_neighbors, reducers."
...
if __name__ == "__main__":
    model = PointMLPForUOP(num_classes=40, embed_dim=64, groups=1, res_expansion=1.0,
                           activation="relu", bias=False, use_xyz=False, normalize="anchor",
                           dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
